...
信号の ON/OFF は `Gate`（音声）と `Note gate`（MIDI）、値の切り替えは `Switch` です。
MIDI キースイッチで分岐させたいときは `Key switch`（送出先の切り替え）と
`Key parameter`（パラメータ値の切り替え）を使います。どちらも行き先／値ごとに
鍵を 1 つ持ち、`Mix` の入力と同じように増減できます。`Note gate` が閉じている間は

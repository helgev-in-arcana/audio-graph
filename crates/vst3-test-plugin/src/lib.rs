...
impl IAudioProcessorTrait for GainProcessor {
    ...
    unsafe fn process(&self, data: *mut ProcessData) -> tresult {
        ...
        if AUDIT_EMIT.swap(false, Ordering::SeqCst) {
            ...
            let id = 0;
            let mut index = 0;
            let queue = ComRef::from_raw(changes.addParameterData(&id, &mut index)).unwrap();
            assert_eq!(queue.addPoint(0, 0.6, &mut index), kResultOk);
        }
...
impl IEditControllerTrait for GainController {
    ...
    unsafe fn setComponentHandler(&self, handler: *mut IComponentHandler) -> tresult {
        AUDIT_HANDLER.store(handler, Ordering::SeqCst);
        AUDIT_CONTROLLER.store(
            if handler.is_null() {
                ptr::null_mut()
            } else {
    ...
    unsafe fn createView(&self, _name: *const c_char) -> *mut IPlugView {
        AUDIT_VIEWS.fetch_add(1, Ordering::SeqCst);
        ComWrapper::new(AuditView)
...
extern "system" fn InitDll() -> bool {
    AUDIT_DEPTH.fetch_add(1, Ordering::SeqCst);
    true
}
...
extern "system" fn ExitDll() -> bool {
    AUDIT_DEPTH.fetch_sub(1, Ordering::SeqCst);
    AUDIT_EXIT_VIEWS.store(AUDIT_VIEWS.load(Ordering::SeqCst), Ordering::SeqCst);
    true
}
...
static AUDIT_HANDLER: std::sync::atomic::AtomicPtr<IComponentHandler> =
...
#[unsafe(no_mangle)]
pub extern "C" fn audit_vst_gui_edit(value: f64) {
    unsafe {
        (*AUDIT_CONTROLLER.load(Ordering::SeqCst)).gain.set(value);
        let handler = ComRef::from_raw(AUDIT_HANDLER.load(Ordering::SeqCst)).unwrap();
        handler.beginEdit(0);
...
pub extern "C" fn audit_vst_depth() -> u32 {
    AUDIT_DEPTH.load(Ordering::SeqCst)
}
#[unsafe(no_mangle)]
pub extern "C" fn audit_vst_exit_views() -> u32 {
    AUDIT_EXIT_VIEWS.load(Ordering::SeqCst)
}
struct AuditView;
impl Class for AuditView {
    type Interfaces = (IPlugView,);
}
impl Drop for AuditView {
    fn drop(&mut self) {
        AUDIT_VIEWS.fetch_sub(1, Ordering::SeqCst);
    }
}
impl IPlugViewTrait for AuditView {
    ...
    unsafe fn onSize(&self, _: *mut ViewRect) -> tresult {
        kResultOk
    }
    unsafe fn onFocus(&self, _: TBool) -> tresult {
        kResultOk
    }
    unsafe fn setFrame(&self, _: *mut IPlugFrame) -> tresult {
        kResultOk
    }
    unsafe fn canResize(&self) -> tresult {
        kResultFalse
    }
    unsafe fn checkSizeConstraint(&self, _: *mut ViewRect) -> tresult {
        kResultOk
    }
}
